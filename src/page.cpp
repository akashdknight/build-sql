...
Page::Page(string tableName, int pageIndex, bool isMatrix)
{
    ...
    {
        Matrix matrix = *matrixCatalogue.getMatrix(tableName);  //here tableName is just a variable which is name of page, can be matix too.
        this->columnCount = matrix.columnCount;
        uint maxRowCount = matrix.maxRowsPerBlock;

        vector<int> row(matrix.colsPerBlockCount[pageIndex], 0);
        this->rows.assign(matrix.rowsPerBlockCount[pageIndex], row);
...
void Page::writePage()
{
    ...
    int row_till = this->rows.size();
    int col_till = this->rows[0].size();
    for (int rowCounter = 0; rowCounter < row_till; rowCounter++)
    {
        for (int columnCounter = 0; columnCounter < col_till; columnCounter++)
        {
            if (columnCounter != 0)
                fout << " ";
...
void Page::writePage(vector<vector<int>> newRows)
{
    logger.log("Page::writePage");
    this->rows = newRows;
    this->rowCount = newRows.size();
    this->columnCount = newRows[0].size();
}

//getter functions for the rows
